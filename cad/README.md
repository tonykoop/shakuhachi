...
- **Current source of truth in-repo:** `shakuhachi_master.scad` plus the workbook and drawing set.
- **What is intentionally missing:** no checked-in SolidWorks part/design-table handoff yet, because the first measured prototype should confirm `δ_utaguchi` before a second CAD master is frozen.
- **Next CAD action after the first build:** export one confirmed render and one dimensioned drawing from the measured geometry, then decide whether a `sw-reference/` drop adds value or just duplicates the OpenSCAD master.
